...
public class ServletVendreUnArticle extends HttpServlet {
	...
	String sdecscription = null;
	String scategorie = null;
	String sphoto = null;
	String fileName = null;
	String sprix = null;
	String srue = null;
	String scode_postal = null;
	String sville = null;
	String sdate_debut = null;
	String sheure_debut = null;
	String sdate_fin = null;
	String sheure_fin = null;
	int intsprix = 0;
	int intscode_postal = 0;
	int intscategorie = 0;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null && !user.getPseudo().isEmpty()) {
			...
			this.getServletContext().getRequestDispatcher("/WEB-INF/pages/VendreUnArticle.jsp").forward(request,
					response);
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * // --> Telecharger photo dans dossier imageArticle // construit le chemin du
		 * répertoire pour enregistrer le fichier téléchargé String uploadFilePath =
		 * request.getServletContext().getRealPath("") + "public" + File.separator +
		 * UPLOAD_DIR;
		 * 
		 * // crée le répertoire de sauvegarde s'il n'existe pas File fileSaveDir = new
		 * File(uploadFilePath); if (!fileSaveDir.exists()) { fileSaveDir.mkdirs(); }
		 * 
		 * /* // Récupère toutes les parties de la requête et les écrit dans le fichier
		 * sur le serveur Part part = request.getPart("sphoto"); fileName =
		 * getFileName(part);
		 * 
		 * if(fileName != null && !fileName.isEmpty()) { part.write(uploadFilePath +
		 * File.separator + fileName); }
		 */

		if ((request.getParameter("sarticle") != null) && !request.getParameter("sarticle").isEmpty()
				&& (request.getParameter("sdecscription") != null) && !request.getParameter("sdecscription").isEmpty()
				&& (request.getParameter("scategorie") != null) && !request.getParameter("scategorie").isEmpty()
				&& (request.getParameter("sprix") != null) && !request.getParameter("sprix").isEmpty()
				&& (request.getParameter("srue") != null) && !request.getParameter("srue").isEmpty()
				&& (request.getParameter("scode_postal") != null) && !request.getParameter("scode_postal").isEmpty()
				&& (request.getParameter("sville") != null) && !request.getParameter("sville").isEmpty()
				&& (request.getParameter("sdate_debut") != null) && !request.getParameter("sdate_debut").isEmpty()
				&& (request.getParameter("sheure_debut") != null) && !request.getParameter("sheure_debut").isEmpty()
				&& (request.getParameter("sdate_fin") != null) && !request.getParameter("sdate_fin").isEmpty()
				&& (request.getParameter("sheure_fin") != null) && !request.getParameter("sheure_fin").isEmpty()) {

			// verifier format date envoyer et en fonction envoyer une erreur si 9:00 au
			// lieu de 09:00
			...
			// Recuperer les dates
			date_debut_enchere = parseStringToLocalDate(request, response, request.getParameter("sdate_debut"),
					request.getParameter("sheure_debut"));
			date_fin_enchere = parseStringToLocalDate(request, response, request.getParameter("sdate_fin"),
					request.getParameter("sheure_fin"));
			...
			if (fileName == null || fileName.isEmpty()) {
				articleVendu = new ArticleVendu(sarticle, sdecscription, date_debut_enchere, date_fin_enchere, intsprix,
						user.getNoUtilisateur(), intscategorie, retrait.getNoArticle());
			} else {
				articleVendu = new ArticleVendu(sarticle, sdecscription, date_debut_enchere, date_fin_enchere, intsprix,
						fileName, user.getNoUtilisateur(), intscategorie, retrait.getNoArticle());
			}
	 ...
	 */
	protected void getParameterAndSetAttribute(HttpServletRequest request, HttpServletResponse response,
	 ...
	 */
	protected void setAutomaticDate(HttpServletRequest request, HttpServletResponse response, String sdate_debut,
			String sheure_debut, String sdate_fin, String sheure_fin) {
	...
	protected LocalDateTime parseStringToLocalDate(HttpServletRequest request, HttpServletResponse response,
		...
		localDate = LocalDate.of(Integer.parseInt(dateString.substring(0, 4)),
				Integer.parseInt(dateString.substring(5, 7)), Integer.parseInt(dateString.substring(8, 10)));

		localTime = LocalTime.of(Integer.parseInt(timeString.substring(0, 2)),
